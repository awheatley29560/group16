...
class Paginator
{
    ...
    private function _previousLink($start)
    {
        if ($this->_currentPage > $start) {
            return '<li><a href="' . $this->_link . '?source=ViewUser&current=' . ($this->_currentPage - 1) . '">' . $this->navigation['pre'] . '</a></li>';
        }
        return '';
    }
    ...
    private function _page($start, $end)
    {
        ...
        for ($i = $start; $i <= $end; $i++) {
            $result .= '<li ' . (($i == $this->_currentPage) ? 'class="disabled"' : "") . '><a href="' . $this->_link . '?source=ViewUser&current=' . $i . '">' . $i . '</a></li>';
        }
        return $result;
    }
    ...
    private function _nextLink($end)
    {
        ...
        if ($this->_currentPage < $end) {
            return '<li><a href="' . $this->_link . '?source=ViewUser&current=' . ($this->_currentPage + 1) . '">' . $this->navigation['next'] . '</a></li>';
        }
        return '';
    }
